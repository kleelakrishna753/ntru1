...
class NtruCipher:
    ...
    def generate_public_key(self, f_poly, g_poly):
        ...
        # Ensure all polynomials are in the correct domain before any operations
        self.f_poly = self.f_poly.set_domain(GF(self.p))
        self.R_poly = self.R_poly.set_domain(GF(self.p))  # Ensure modulus is in the same field

        # Debug logs for tracking
        log.debug("Trying to invert: {}".format(self.f_poly))
        
        # Invert polynomials f_poly and f_q_poly
        ...
        # Ensure that all polynomials are in the same field
        self.f_p_poly = self.f_p_poly.set_domain(GF(self.p))
        self.f_q_poly = self.f_q_poly.set_domain(GF(self.p))
        self.g_poly = self.g_poly.set_domain(GF(self.p))  # Ensure g_poly is in the same field as f_poly
        
        # Perform operations on the polynomials
        log.debug("f*f_p mod (x^n - 1): {}".format((self.f_poly * self.f_p_poly) % self.R_poly))
        log.debug("f*f_q mod (x^n - 1): {}".format((self.f_poly * self.f_q_poly) % self.R_poly))

        p_f_q_poly = (self.p * self.f_q_poly).trunc(self.q)
        log.debug("p_f_q: {}".format(p_f_q_poly))
        
        # Ensure that both p_f_q_poly and g_poly are in the same field
        p_f_q_poly = p_f_q_poly.set_domain(GF(self.p))
        
        # Additional operations for h_poly
