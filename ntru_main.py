...
Usage:
  ntru.py enc PUB_KEY_FILE [--poly-input] [--poly-output] [--block] [FILE]
  ntru.py dec PRIV_KEY_FILE [--poly-input] [--poly-output] [--block] [FILE]
  ntru.py gen N P Q PRIV_KEY_FILE PUB_KEY_FILE [--debug | --verbose]
  ntru.py (-h | --help)
  ntru.py --version

Options:
  N           The polynomial degree for key generation.
  P           The parameter P for NTRU.
  Q           The parameter Q for NTRU.
...
def generate(N, p, q, priv_key_file, pub_key_file):
    log.info(f"Generating keys with N={N}, p={p}, q={q}")
    ...
    np.savez(priv_key_file, N=N, p=p, q=q, f=f, f_p=f_p)
    log.info("Private key saved to {} file".format(priv_key_file))
    np.savez(pub_key_file, N=N, p=p, q=q, h=h)
